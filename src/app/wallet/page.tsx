...
export default function WalletPage() {
  const { balance, bonusBalance, wageringRequirement, betHistory } = useGameStore();
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <h1 className="text-3xl font-heading font-bold gold-text">💰 Wallet</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card p-5"><p className="text-sm text-gray-400">Cash Balance</p><p className="text-2xl font-bold text-neon-400">${balance.toLocaleString()}</p></div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto hide-scrollbar">
            {betHistory.slice(0, 10).map((bet, i) => (
              <div key={i} className="flex justify-between text-sm py-1 border-b border-white/5">
                ...
                <span className={bet.outcome === "win" ? "text-neon-400" : "text-rose-500"}>${bet.profit}</span>
              </div>
            ))}
          </div>
        </div>
