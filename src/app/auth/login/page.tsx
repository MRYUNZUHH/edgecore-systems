    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-800 to-orange-700 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <motion.div initial={{ scale:0.8, opacity:0 }} animate={{ scale:1, opacity:1 }} className="w-full max-w-md bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-white/70 mt-2">Your luck starts here</p>
        </div>
        {err && <p className="text-red-300 text-sm mb-4">{err}</p>}
        <input value={u} onChange={e => setU(e.target.value)} placeholder="Username" className="w-full p-3 bg-white/20 border border-white/30 rounded-xl text-white placeholder-white/50 mb-3 focus:outline-none focus:border-gold-400" />
        <input value={p} onChange={e => setP(e.target.value)} type="password" placeholder="Password" className="w-full p-3 bg-white/20 border border-white/30 rounded-xl text-white placeholder-white/50 mb-6 focus:outline-none focus:border-gold-400" />
        <button onClick={handleLogin} className="btn-primary w-full py-3 text-lg">Login</button>
        <p className="text-center text-white/70 mt-4 text-sm">New? <a href="/auth/signup" className="text-gold-400 font-bold">Sign up</a></p>
      </motion.div>
    </div>
  );
}
